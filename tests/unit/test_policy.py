...
class TestApplyPolicyFilters:
    def test_allow_list(self):
        ...
        result = apply_policy_filters(
            tools, policy={"allow": ["fine_tune", "list_training_jobs"]}
        )
...
class TestCustomPersonas:
    def test_custom_persona_from_policy_file(self):
        policy = {
            "personas": {"custom-role": {"tools": ["fine_tune", "list_training_jobs"]}}
        }
        with patch("kubeflow_mcp.core.policy._load_policy_file", return_value=policy):
            reload_policy()
    ...
    def test_custom_persona_with_inheritance(self):
        policy = {
            "personas": {
                "extended-readonly": {"inherit": "readonly", "tools": ["fine_tune"]}
            }
        }
        with patch("kubeflow_mcp.core.policy._load_policy_file", return_value=policy):
            reload_policy()
...
class TestPolicyFileLoading:
    def test_load_from_yaml(self, tmp_path):
        ...
        with patch("kubeflow_mcp.core.policy.POLICY_PATHS", [policy_file]):
            assert get_allowed_namespaces() == ["ml-dev"]

    def test_missing_file_returns_none(self):
        with patch(
            "kubeflow_mcp.core.policy.POLICY_PATHS", [Path("/nonexistent/policy.yaml")]
        ):
            assert get_allowed_namespaces() is None
...
class TestReloadPolicy:
    def test_picks_up_new_personas(self):
        ...
        with patch(
            "kubeflow_mcp.core.policy._load_policy_file", return_value=policy_v1
        ):
            reload_policy()
            assert "list_training_jobs" in get_allowed_tools("role-v1")

        with patch(
            "kubeflow_mcp.core.policy._load_policy_file", return_value=policy_v2
        ):
            reload_policy()
            with pytest.raises(ValueError, match="Unknown persona"):
