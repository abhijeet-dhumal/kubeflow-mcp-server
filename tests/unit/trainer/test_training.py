...
class TestRunCustomTraining:
    ...
    def test_preview_image_and_pip_index_urls(self):
        ...
        assert result["config"]["pip_index_urls"] == [
    ...
    def test_preview_gpu_per_node_default_resources(self):
        result = run_custom_training(
            script="print('hi')", gpu_per_node=2, confirmed=False
        )
        assert result["config"]["resources_per_node"] == {"gpu": 2}

    def test_preview_cpu_only_no_resources(self):
        result = run_custom_training(
            script="print('hi')", gpu_per_node=0, confirmed=False
        )
    ...
    def test_rejects_dangerous_import_os(self):
        result = run_custom_training(
            script="import os\nprint(os.getcwd())", confirmed=True
        )
        assert result["success"] is False
    ...
    def test_rejects_subprocess(self):
        result = run_custom_training(
            script="import subprocess\nsubprocess.run(['ls'])", confirmed=True
        )
        assert result["success"] is False

    def test_rejects_eval(self):
        result = run_custom_training(script="eval('print(1)')", confirmed=True)
        assert result["success"] is False

    def test_rejects_exec(self):
        result = run_custom_training(script="exec('x = 1')", confirmed=True)
        assert result["success"] is False
...
"""
    ...
    def test_rejects_invalid_job_name(self):
        result = run_custom_training(
            script="print('hello')", name="INVALID_NAME", confirmed=True
        )
        assert result["success"] is False
        assert (
            "validation" in result["error"].lower() or "name" in result["error"].lower()
        )
    ...
    def test_submit_train_func_isolated_namespace(self, mock_get_client):
        ...
        assert not hasattr(_mod, "isolated_sentinel"), (
            "Script leaked into module namespace"
        )
...
class TestRunContainerTraining:
    ...
    def test_preview_cpu_only(self):
        result = run_container_training(
            image="python:3.10", gpu_per_node=0, confirmed=False
        )
    ...
    def test_submit_affinity_in_patch(self, mock_get_client):
        ...
        run_container_training(
            image="myrepo/trainer:v1", affinity=affinity, confirmed=True
        )
