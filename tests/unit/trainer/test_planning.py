...
class TestGetClusterResources:
    ...
    def test_k8s_unavailable(self):
        with patch(
            "kubernetes.config.load_config", side_effect=Exception("No cluster")
        ):
            result = get_cluster_resources()
...
class TestEstimateResources:
    ...
    def test_large_model(self, mock_hf_info):
        ...
        assert (
            result["data"]["total_gpu"] >= 2
            or "80GB" in result["data"]["gpu_type_recommended"]
        )
...
class TestEstimateFromParams:
    def test_1b_model(self):
        ...
        assert "8GB" in result["gpu_type"]
        assert result["gpu_memory_gb"] <= 8
    ...
    def test_70b_model(self):
        ...
        assert result["gpu_memory_gb"] > 80
